...
def ai_command(command: AICommand):
    ...
    try:
        ...
        data = json.loads(cleaned_reply[start:end])
    except Exception:
        return {"message": "AI returned invalid response", "raw": cleaned_reply}
    ...
    # =========================
        ...
        if status:
            ...
            if status in ["open", "opened", "new"]:
                status = "Open"
            elif status in ["in progress", "progress", "working"]:
                status = "In Progress"
            elif status in ["closed", "close", "done"]:
                status = "Closed"
            elif status in ["all", "all status"]:
                status = None
